...
import type { CharacterDetailDto } from "@pixaura/shared-types";
...
const mainImage = computed(() => {
  if (props.character?.images?.frontView) {
    return props.character.images.frontView;
  }
  // 兼容旧的 images 字段结构
  if (props.character?.images) {
    const images = props.character.images as Record<string, unknown>;
...
const executeGenerate = async (prompt?: string) => {
  ...
  try {
    const response = await fetch(
      `/api/characters/${props.character?.id}/images/generate`,
      {
        method: "POST",
