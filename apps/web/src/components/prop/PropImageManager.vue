...
import type { PropDetailDto } from "@pixaura/shared-types";

interface Props {
  prop: PropDetailDto | null;
  projectId: string;
}
...
// 获取主图（使用 frontView 作为主图）
const mainImage = computed(() => {
  if (props.prop?.images?.frontView) {
    return props.prop.images.frontView;
  }
  // 兼容旧的 images 字段结构
  if (props.prop?.images) {
    const images = props.prop.images as Record<string, unknown>;
    if (images.main && Array.isArray(images.main) && images.main.length > 0) {
...
const executeGenerate = async (prompt?: string) => {
  ...
  try {
    const response = await fetch(
      `/api/props/${props.prop?.id}/images/generate`,
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          type: "front_view", // 只生成主图
